...
public class CryptoUtils {
    ...
    public byte[] aesEncrypt(String sSrc) {
        ...
        try {
           ...
           MSLoggerUtils.getInstance().e(TAG,"aesEncrypt encrypt error");
           return null;
        }
        if (encrypted == null) {
            MSLoggerUtils.getInstance().e(TAG, "aesEncrypt The encrypted data is empty");
    ...
    public String aesDecrypt(byte[] sSrc) {
        ...
        try {
            ...
            MSLoggerUtils.getInstance().e(TAG, "aesDecrypt Decryption error");
        }
    ...
    public String digestMD5(String password) {
        try {
            ...
            for (byte b : bytes) {
                ...
                String result = Integer.toHexString(c); //把十进制的数转换成十六进制
                if (result.length() < 2) {
    ...
    public boolean checkRSASign(String content, String sign) {
        try {
            String publicKey = MSIMApplication.getInstance().getRSAPublicKey();
