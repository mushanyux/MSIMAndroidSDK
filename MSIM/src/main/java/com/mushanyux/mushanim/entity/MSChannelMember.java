...
public class MSChannelMember implements Parcelable {
    //自增ID
    public long id;
    //频道id
    ...
    protected MSChannelMember(Parcel in) {
        id = in.readLong();
        status = in.readInt();
        ...
        extraMap = MSCommonUtils.str2HashMap(extraStr);
    }
    ...
    public static final Creator<MSChannelMember> CREATOR = new Creator<MSChannelMember>() {
        @Override
        public MSChannelMember createFromParcel(Parcel in) {
            return new MSChannelMember(in);
        }

        @Override
        public MSChannelMember[] newArray(int size) {
            return new MSChannelMember[size];
        }
    };
}
