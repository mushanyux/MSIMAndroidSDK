...
public class MSChannel implements Parcelable {
    //自增ID
    public long id;
    //频道ID
...
//    public int mainDeviceFlag;
    ...
    public MSChannel(String channelID, byte channelType) {
    ...
    protected MSChannel(Parcel in) {
        ...
        localExtra = MSCommonUtils.str2HashMap(localStr);
        String remoteStr = in.readString();
        remoteExtraMap = MSCommonUtils.str2HashMap(remoteStr);
    }
    ...
    public static final Creator<MSChannel> CREATOR = new Creator<MSChannel>() {
        @Override
        public MSChannel createFromParcel(Parcel in) {
            return new MSChannel(in);
        }

        @Override
        public MSChannel[] newArray(int size) {
            return new MSChannel[size];
        }
    };
}
