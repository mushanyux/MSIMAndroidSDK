...
import com.mushanyux.mushanim.utils.BigTypeUtils;
...
public class MSRead {
    ...
    public int readPacketType() throws IOException {
        ...
        return MSTypeUtils.getInstance().getHeight4(header[0]);
    }

    public int readRemainingLength() {
        return MSTypeUtils.getInstance().bytes2Length(inputStream);
    }
    ...
    public String readString() throws IOException {
        ...
        if (read == -1) return "";
        return MSTypeUtils.getInstance().bytesToString(strByte);
    }
    ...
    public String readPayload() throws IOException {
        ...
        if (read == -1) return "";
        return MSTypeUtils.getInstance().bytesToString(payload);
    }
}
